...
public:
    ...
    Contract(const string sec_type, const string symbol, const string expiry, long multiplier, const string contract_month,
...
public:
    ...
    Order(const string order_type, const string account, Contract &contract, const string action, long total_quantity) :
            order_type(order_type), account(account), contract(contract), action(action), total_quantity(total_quantity) {};
